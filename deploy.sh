...
EXPECTED_HEAD="${2:-198_near_you_country_and_indexes}"
ROOT="/opt/teivaka"
...
[ "${ENT:-0}" = "2" ] && ok "production_units enterprise_type + unit_of_measure present (130)" || bad "production_units keystone columns missing (${ENT}/2 — migration 130)"
NONULL="$($PSQL -c "SELECT count(*) FROM tenant.production_units WHERE enterprise_type IS NULL;" 2>/dev/null | tr -d '[:space:]')"
