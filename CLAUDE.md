...
- Alembic migrations 001 through current head `201_feed_signals` (single head as of 2026-06-29; adds the append-only community.feed_signals engagement log — Feed v2 Slice 1 ranking/flywheel foundation). Never edit a stamped migration — write a new one. Verify head with `docker exec teivaka_api alembic current`. NOTE (B78 ordering): `alembic stamp <new_rev>` must run AFTER the backend image is rebuilt — the container only gets the new migration file on rebuild; stamping before the rebuild fails with "Can't locate revision" (surfaced 2026-06-22, migration 159).
