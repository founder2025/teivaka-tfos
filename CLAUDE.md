...
- Alembic migrations 001 through current head `202_feed_post_counters` (single head as of 2026-06-29; denormalized like/reply/repost counts on community.feed_posts + backfill — Feed v2 Slice 1b, kills the feed-read N+1; recount-on-write keeps them accurate; MUST apply migration before the new code serves). Never edit a stamped migration — write a new one. Verify head with `docker exec teivaka_api alembic current`. NOTE (B78 ordering): `alembic stamp <new_rev>` must run AFTER the backend image is rebuilt — the container only gets the new migration file on rebuild; stamping before the rebuild fails with "Can't locate revision" (surfaced 2026-06-22, migration 159).
