...
- Alembic migrations 001 through current head `203_feed_rank_score` (single head as of 2026-06-29; precomputed `rank_score`/`ranked_at` + partial index on community.feed_posts — Feed v2 Slice 2a, turns the feed read's on-demand full sort into an index-ordered scan; scored at INSERT + refreshed by the `recompute_feed_rank` beat task every 10 min; MUST apply migration before the new code serves. Prior head `202_feed_post_counters` = denormalized like/reply/repost counts, Feed v2 Slice 1b). Never edit a stamped migration — write a new one. Verify head with `docker exec teivaka_api alembic current`. NOTE (B78 ordering): `alembic stamp <new_rev>` must run AFTER the backend image is rebuilt — the container only gets the new migration file on rebuild; stamping before the rebuild fails with "Can't locate revision" (surfaced 2026-06-22, migration 159).
