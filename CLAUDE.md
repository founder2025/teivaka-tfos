...
- Alembic migrations 001 through current head `207_user_trust_cache` (single head as of 2026-06-30; adds community.user_trust — Trust Ladder Slice 2, a denormalized per-user trust-level cache (no RLS, community-owned; NOT a tenant.users column) refreshed hourly by the `recompute_trust_levels` beat task (Strike #95 two-stage scan — audit.events has strict tenant-RLS). Powers cheap trust badges on listings (seller_trust_level), directory (trust_level), and feed authors (author_trust_level); level decided by the shared `trust.level_from_signals`. Prior head `206_user_suspensions` = moderator kill-switch, T&S Slice 3). Never edit a stamped migration — write a new one. Verify head with `docker exec teivaka_api alembic current`. NOTE (B78 ordering): `alembic stamp <new_rev>` must run AFTER the backend image is rebuilt — the container only gets the new migration file on rebuild; stamping before the rebuild fails with "Can't locate revision" (surfaced 2026-06-22, migration 159).
