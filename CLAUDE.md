...
- Alembic migrations 001 through current head `205_feed_flags_auto_source` (single head as of 2026-06-30; makes community.feed_flags.reporter_user_id nullable so the automated scam/spam classifier can file machine flags — Trust & Safety Slice 2, deterministic `app/core/content_safety.py` auto-flags obvious scams from post-create + listing-create into the queue with category='AUTO', flag-for-review never auto-hide; velocity caps already covered every write vector. Prior head `204_moderation_queue_unify` = unified moderation queue, T&S Slice 1). Never edit a stamped migration — write a new one. Verify head with `docker exec teivaka_api alembic current`. NOTE (B78 ordering): `alembic stamp <new_rev>` must run AFTER the backend image is rebuilt — the container only gets the new migration file on rebuild; stamping before the rebuild fails with "Can't locate revision" (surfaced 2026-06-22, migration 159).
