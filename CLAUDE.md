...
- Alembic migrations 001 through current head `204_moderation_queue_unify` (single head as of 2026-06-30; generalizes community.feed_flags into ONE moderation queue via target_type/target_id — Trust & Safety Slice 1, so chat reports + marketplace listing reports surface to moderators (previously chat_reports was unread + listings unreportable); drops the legacy post_id-OR-reply_id CHECK; listing takedown via admin action REMOVE_LISTING. Prior head `203_feed_rank_score` = precomputed rank_score + index, Feed v2 Slice 2a). Never edit a stamped migration — write a new one. Verify head with `docker exec teivaka_api alembic current`. NOTE (B78 ordering): `alembic stamp <new_rev>` must run AFTER the backend image is rebuilt — the container only gets the new migration file on rebuild; stamping before the rebuild fails with "Can't locate revision" (surfaced 2026-06-22, migration 159).
