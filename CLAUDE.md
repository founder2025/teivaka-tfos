...
- Alembic migrations 001 through current head `206_user_suspensions` (single head as of 2026-06-30; adds community.user_suspensions — Trust & Safety Slice 3, the moderator kill-switch. Suspension is a community-level fact (no RLS, NOT a tenant.users.is_active write) enforced at community-WRITE time via `assert_not_suspended` in community_guard (fail-open); blocks posting/listing/DM/etc., never blocks reads; fully reversible (delete row). Admin action SUSPEND_USER on any flag + GET /suspensions + POST /unsuspend/{id}. Prior head `205_feed_flags_auto_source` = auto-classifier machine flags, T&S Slice 2). Never edit a stamped migration — write a new one. Verify head with `docker exec teivaka_api alembic current`. NOTE (B78 ordering): `alembic stamp <new_rev>` must run AFTER the backend image is rebuilt — the container only gets the new migration file on rebuild; stamping before the rebuild fails with "Can't locate revision" (surfaced 2026-06-22, migration 159).
