...
- Last migration: `162_capture_audit_events` (single head; slice-2 audit-hole sweep — `161_register_create_audit` seeds BUYER_ADDED/EQUIPMENT_ADDED/INPUT_ADDED/WORKER_ADDED, `162` seeds ATTENDANCE_LOGGED + re-asserts WORKER_CHECKIN/INPUT_USED_ADJUSTMENT, both rebuild the audit.events CHECK from the catalog; applied-as-owner per Strike #123 on 2026-06-23). Prior: `160_cash_ledger_evidence` (B92 receipt evidence); `159_field_events_evidence_v2` (Evidence v2 voice+witness); `158_field_events_grade_deliv` (GRADE/DELIVERY verbs); debt-sweep chain `151`→`152_merge_feed_audience`→`153_revoke_audit_mutations`→`154_users_rls_permissive`→`155_poultry_created_by_fk`→`156_cycle_layer_not_null`→`157_drop_tenant_mode`.
...
**(+) FAB = Universal Capture Engine, two-vertical (built + deployed 2026-06-22, prototype-(+)-parity commission).** The old 11-pillar tile wall is GONE. The (+) now opens **🌱 Plant-based · 🐾 Animal-based · 💼 Whole-farm** (`LogSheet.jsx`). Plant + both animal sub-flows render the config-driven `frontend/src/capture/CaptureEngine.jsx` (verb-first, Universal Event Form: anchors card + date/time + Evidence + notes + "about to record" preview); only Whole-farm (Money/Notes) is still routed tiles. Configs: `capture/config/crops.js` (30 events incl. lifecycle link-verbs to CycleNew/NurseryNew/HarvestNew), `animal-poultry.js` (18 events, flock-anchored via `GET /flocks`, library pickers for feed/vaccine), `animal-livestock.js` (8 events, farm-anchored, species-led). Engine is context-driven (`config.context` = loader/idKey/buildAnchors); input types: choice/multichoice/number/text/chemical(WHD)/library(farm_libraries UUID); `spec.autofillDate` + `spec.validate` hooks. **Evidence Architecture v2 LIVE**: Photo+GPS+Voice(MediaRecorder)+Witness, photo/voice SHA-256-hashed into the audit chain (migration 159). Every verb path writes exactly one real `audit.events` row via `/events` (Universal Event Form Contract). Verified on prod: crop FIELD_OBSERVATION+witness, GRADING/DELIVERY, PEST/DISEASE_CONFIRMED, LIVESTOCK_ACQUIRED all `success`. Still tiles/legacy: Whole-farm Money (B76/B87 — engine candidate); optional poultry FK pickers (buyer/supplier/disinfectant); HARVEST_LOGGED on legacy /harvests (B75). Browser-only unverified: voice mic capture, poultry verbs (need a flock placed first).
...
- B75: **CLOSED — WON'T-DO, BY DESIGN (Operator-ratified 2026-06-24).** Harvest stays on its dedicated `/api/v1/harvests` path; it is NOT unified onto polymorphic `/events`. Rationale: harvest is the WHD hard-gate (Inviolable #2) path — three layers (FOUNDER-only override + `tenant.harvest_compliance_overrides` trail; `harvest_service.check_chemical_compliance()` → 409; DB trigger `tenant.enforce_harvest_compliance` (015a) `RAISE EXCEPTION CHEMICAL_COMPLIANCE_VIOLATION` on every `INSERT INTO tenant.harvest_log`) + RULE-038 FOUNDER-override critical alert + Bank Evidence reads `harvest_log` for production evidence. `HARVEST_LOGGED` is intentionally NOT in EVENT_TYPE_REGISTRY. Unifying onto `/events` would mean re-routing the WHD gate for pure architectural symmetry — high risk to the single safety+bankability guarantee, zero farmer-visible benefit (the (+) harvest verb already hands off to HarvestNew end-to-end with full enforcement). A dedicated compliance-enforced endpoint is correct design here, not debt. If symmetry is ever wanted, the ONLY acceptable route is DELEGATE (register `HARVEST_LOGGED` so `/events` *calls* the existing `harvest_service` — door-only, same gate/table/trigger), staged with a tested cutover + rollback — but it remains avoidable risk on the hottest path and is not planned. Original: HARVEST_LOGGED still on /api/v1/harvests legacy path. Migrate to polymorphic /events with field_events backing as part of Strike #97 scope. Mirrors Strike #96 pattern: CATALOG_TO_FIELD_VERB extended with `HARVEST_LOGGED → HARVEST_PARTIAL` or `HARVEST_FINAL` based on payload flag.
...
- B83: Weekly AI insights — rebuild before re-enabling. `ai-insights-weekly` beat entry DISABLED 2026-06-21 (Cluster 1 debt sweep): `generate_weekly_insights` crash-looped at the RLS boundary (`ai_worker.py:129` queried `tenant.farms` as non-bypass `teivaka_app` with no `app.tenant_id` set → `unrecognized configuration parameter "app.tenant_id"`), so it never reached the model call (actual spend ≈ $0), and even on success only `logger.info`'d the result — output was never persisted. The task body is now doctrine-clean (routes via the OpenClaw `bridge_chat`, no direct Anthropic call). Proper rebuild before re-enabling: (1) two-stage tenant scan per Strike #95 (iterate `tenant.tenants`, then per-tenant `SET LOCAL app.tenant_id`), (2) real persistence (store as a community post / KB note, not a log line), (3) a prototype-designed UI surface. Re-enable = uncomment `celery_app.py` ~line 115. Surfaced during Cluster 1 verify (Strike #78 full-lifecycle smoke).
- B84: RLS defense-in-depth hardening (NOT an active leak — deferred from Cluster 3, debt sweep 2026-06-21). Several paths run tenant queries without RLS context and rely on hand-written WHERE scoping rather than the policy: (1) `me.py` uses `get_db` (NULL context) for ~13 endpoints — safe today because every query filters by the authenticated `tenant_id` or globally-unique `user_id`, but RLS should be the backstop (switch to `get_tenant_db`); (2) `deps/tasks.py:233` `set_tenant_context` uses session-scoped `set_config(...,false)` — flip to txn-local `true` AFTER auditing every caller wraps in a transaction (else NULL-context queries result); (3) `maintenance_worker.py` does cross-tenant writes claiming BYPASSRLS it doesn't have (B73 family) — bring onto the `with_rls` two-stage scan. No active cross-tenant leak found (verified: queries are WHERE-scoped by auth identity). Do before widening multi-tenant aggressively; highest-care (live auth/RLS), stage each per-endpoint with rollback.
- B85: Low-value dead-file cleanup deferred from Cluster 5 (debt sweep 2026-06-21). (1) Stale `requirements.txt` — there are TWO (`11_application_code/requirements.txt` + `04_environment/requirements.txt`) and the Dockerfile comment references the name; the deployed image installs `requirements-api.txt`. Confirm which bare file is truly unused before deleting (the `11_application_code` one lists pandas/boto3/supabase with 0 import sites and is the prime suspect). (2) Duplicated 4.3MB prototype HTML (`docs/TFOS_MyFarm_Prototype_v263_20260608.html` ≡ `11_application_code/app/static/prototype_v263.html`) — could be build-generated from the canonical copy instead of committed twice. Both pure hygiene, zero functional impact.
- B86: **CLOSED 2026-06-21 (Cluster 4.2).** `tenant.production_cycles.layer` is now NOT NULL — cycle_service.resolve_layer() enforces a layer at both creation paths (caller value → shared.productions.suggested_layer → borderline 4xx), migration 156 backfilled existing NULLs + SET NOT NULL. Original note: Enforce layer NOT NULL (flipping alone would have broken cycle creation). Both creation paths default layer to NULL: `cycle_service.py:195` (`layer: Optional[str]=None`) and `events.py:1155` (`payload_dict.get("layer")`). Safe 3-step sequence: (1) backend — when layer is None at creation, resolve from `shared.productions.suggested_layer` (Strike #103 seeds it), rejecting only the 7 true borderlines that have no suggestion (highest care: live cycle-creation surface, stage with rollback); (2) backfill existing NULLs via the real path — the Strike #104a backfill banner / `PATCH /cycles/{id}/classify-layer` (as of 2026-06-21 the only NULL is `CYC-F001-A0EE-PU001-2026-001` CASSAVA → FOOD_SECURITY, suggested+doctrine-explicit); (3) migration `ALTER COLUMN layer SET NOT NULL` (apply-as-owner). Satisfies Strike #101/#103 layer-NOT-NULL mandate. 4.1 (created_by FK) already shipped as migration 155.
...
- B91: **CLOSED 2026-06-22.** Nursery (+) action now audit-chain compliant — `log_nursery_batch` emits `NURSERY_BATCH_CREATED` inside the same `get_rls_db` transaction as the `nursery_batches` INSERT (atomic; verified on prod, batch NRS-98DAE9 created + audit committed). Original: `POST /api/v1/nursery` (`routers/nursery.py:71`) persisted `tenant.nursery_batches` but did NOT call `emit_audit_event` — so the (+) "Nursery & seedlings" action (surfaced via the Capture Engine link verb, slice 2 of the prototype-(+) parity build 2026-06-22) writes a real record but no `audit.events` row, violating the Universal Event Form Contract (one (+) action → one audit row). Predates the contract. Fix: add an `emit_audit_event(event_type='NURSERY_BATCH_CREATED', ...)` to `log_nursery_batch` inside the same txn (mirror `cycle_service.create_cycle`'s audit emit), anchored to farm_id/pu(none)/batch. Then nursery germination/ready updates (GERMINATION_LOGGED/NURSERY_READY) should also emit on the PATCH path once added. No new table; the register stays the source of truth, audit becomes the spine. Surfaced slice 2.

- B92: **CLOSED 2026-06-23.** Migration 160 added photo/voice/witness/GPS columns to `tenant.cash_ledger`; `cash.py` accepts + SHA-256-hashes photo/voice into the CASH_LOGGED audit payload; engine submit-adapter passes evidence to `moneyConfig.buildBody`; `moneyConfig.evidence` flipped true. Verified on prod (CSH-20260623-C263 with witness persisted + audit hash). Original: Receipt-photo evidence on Money. The Whole-farm Money capture (Capture Engine → `POST /api/v1/cash-ledger`) is audit-compliant (emits `CASH_LOGGED`) but carried NO evidence — the engine's evidence card is hidden for money (`moneyConfig.evidence:false`) because `tenant.cash_ledger` has no evidence columns. The prototype's cash form shows Photo/etc, and a receipt photo on an expense is high-value Bank Evidence. Fix: add `photo_url/photo_sha256/photo_byte_size` (+ optionally voice/witness) to `tenant.cash_ledger` via migration, accept them on `CashLedgerCreate`, hash photo server-side (mirror events.py `_hash_local_photo`), fold `photo_sha256` into the CASH_LOGGED audit payload; then flip `moneyConfig.evidence` back on. No active bug — money records are complete + verifiable; this adds the receipt layer. Surfaced Whole-farm Money slice.

- B93: **CLOSED 2026-06-24.** Two-part fix shipped. (1) Log redaction — a `_SecretRedactingFilter` on the uvicorn/gunicorn access loggers + root handlers rewrites `(access_token|refresh_token|token|ticket)=<value>` → `=REDACTED` before any line is written (verified in prod). (2) Root elimination — SSE auth moved OFF the query string: `POST /api/v1/community/chat/stream-ticket` (header-authed) mints a 30s single-use opaque ticket in Redis (`stream_ticket:<t>`), the middleware redeems it (GET+DELETE) for both stream paths (`/api/v1/tis/stream` + `/api/v1/community/chat/stream`) and `?access_token=` acceptance was REMOVED entirely. Frontend `utils/streamTicket.js` (`openTicketedStream`) mints a fresh ticket per (re)connect with capped backoff (native EventSource auto-reconnect can't reuse a single-use ticket); ChatWidget + useTisSse migrated. Bonus: fixed the pre-existing chat-stream 401 (middleware only accepted query auth for `/tis/stream`, never chat). No JWT now travels in any URL/log. Original: The community chat SSE endpoint takes the JWT in the URL query string — `GET /api/v1/community/chat/stream?access_token=<JWT>` — so every request writes a valid access token into the uvicorn/docker stdout logs (observed plain in `docker logs teivaka_api` 2026-06-24, role=FOUNDER token). Query-string tokens also land in proxy/CDN logs, browser history, and Referer headers. Risk: anyone with log access can replay the token until expiry. Fix: move SSE auth off the query string — use a short-lived single-use stream ticket (POST to mint, pass the opaque ticket in the URL) OR an `EventSource` polyfill that sends the `Authorization` header OR a cookie-based auth for the stream; then scrub tokens from access logs (uvicorn log filter / Caddy log redaction of `access_token=`). Also audit any other endpoints taking `access_token`/secrets via query string. No data loss; standing exposure while unfixed. Surfaced during network-map debugging (Slice 3).
...
- Alembic migrations 001 through current head `210_featured_placements` (single head as of 2026-07-02; adds community.featured_placements — WH4 "Boost your listing": a trust-gated member features one OPEN job/service for 7 days, sorted top of Find work/Earn with a Featured label, capped 1 active, no payment faked. Prior head `209_marketplace_reviews`; adds community.marketplace_reviews + avg_rating/review_count on community.user_trust — Marketplace M3. A review is buyer-of-a-DONE-order-only, one per order (unbuyable, non-anonymous); aggregates ride the user_trust cache so ★ shows on listings via the existing join; reviews feed the trust ladder via trust.level_from_signals (reviews>=3 counts toward TRUSTED). POST /marketplace/orders/{id}/review + GET /sellers/{id}/reviews; recount-on-write + recompute_trust_levels maintains it. Prior head `208_marketplace_orders_ledger` = order ledger, Marketplace M2). Never edit a stamped migration — write a new one. Verify head with `docker exec teivaka_api alembic current`. NOTE (B78 ordering): `alembic stamp <new_rev>` must run AFTER the backend image is rebuilt — the container only gets the new migration file on rebuild; stamping before the rebuild fails with "Can't locate revision" (surfaced 2026-06-22, migration 159).
