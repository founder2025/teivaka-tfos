...
- Alembic migrations 001 through current head `208_marketplace_orders_ledger` (single head as of 2026-06-30; adds community.marketplace_orders — Marketplace M2, a cross-tenant order/enquiry ledger (no RLS) so the BUYER can see + track orders and every category has an on-platform action + shared status Requested→Accepted→Done. Wired into marketplace.py order (produce/inputs→ACCEPTED) + new /enquire (livestock/tools/wanted→REQUESTED) + /orders/{id}/status + /my-orders. Prior head `207_user_trust_cache` = trust-level cache, Trust Ladder S2). Never edit a stamped migration — write a new one. Verify head with `docker exec teivaka_api alembic current`. NOTE (B78 ordering): `alembic stamp <new_rev>` must run AFTER the backend image is rebuilt — the container only gets the new migration file on rebuild; stamping before the rebuild fails with "Can't locate revision" (surfaced 2026-06-22, migration 159).
