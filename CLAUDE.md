...
- Last migration: `159_field_events_evidence_v2` (single head; Evidence Architecture v2 — adds voice_url/voice_sha256/voice_byte_size/witness_name/witness_contact to `tenant.field_events` so voice + witness evidence persist alongside photo/GPS; down_revision `158_field_events_grade_deliv`; applied-as-owner per Strike #123 on 2026-06-22). Prior: `158_field_events_grade_deliv` (GRADE/DELIVERY verbs); debt-sweep chain `151`→`152_merge_feed_audience`→`153_revoke_audit_mutations`→`154_users_rls_permissive`→`155_poultry_created_by_fk`→`156_cycle_layer_not_null`→`157_drop_tenant_mode`.
...
- B92: Receipt-photo evidence on Money. The Whole-farm Money capture (Capture Engine → `POST /api/v1/cash-ledger`, 2026-06-22) is audit-compliant (emits `CASH_LOGGED`) but carries NO evidence — the engine's evidence card is hidden for money (`moneyConfig.evidence:false`) because `tenant.cash_ledger` has no evidence columns. The prototype's cash form shows Photo/etc, and a receipt photo on an expense is high-value Bank Evidence. Fix: add `photo_url/photo_sha256/photo_byte_size` (+ optionally voice/witness) to `tenant.cash_ledger` via migration, accept them on `CashLedgerCreate`, hash photo server-side (mirror events.py `_hash_local_photo`), fold `photo_sha256` into the CASH_LOGGED audit payload; then flip `moneyConfig.evidence` back on. No active bug — money records are complete + verifiable; this adds the receipt layer. Surfaced Whole-farm Money slice.
