...
| `tenant.users` policy | **permissive-on-NULL** (`current_setting IS NULL OR '' OR match`) | C3 sub-finding resolved: live policy is permissive (auth works), but **source/migrations would deploy STRICT** → fresh-deploy/DR breaks login (untracked drift). → ✅ **RESOLVED 2026-06-21 (Cluster 3.2):** mig `154_users_rls_permissive` codifies the live policy verbatim; stamped on prod (live policy untouched), greenfield now creates permissive. **CORRECTION:** the `me.py`/GUC "cross-tenant exposure" is NOT an active leak — every query is WHERE-scoped by the authenticated `tenant_id`/unique `user_id`; it's defense-in-depth fragility → filed B84 (not urgent). |
| **audit.events grants → teivaka_app** | **INSERT, SELECT, UPDATE, DELETE** | **NEW 🔴 — UPDATE/DELETE must not be granted.** Immutability triggers (`023:169-192`) currently hold the line; defense-in-depth broken. Fix: `REVOKE UPDATE, DELETE ON audit.events FROM teivaka_app` (Cluster 3). → ✅ **RESOLVED 2026-06-21 (Cluster 3.1):** mig `153_revoke_audit_mutations` applied-as-owner + stamped on prod; grants now INSERT+SELECT only; both guard triggers intact. |
| `tenant.inputs` rows | **1** | H1 refined: seed never ran on prod → prod fine; 074 break is greenfield/DR-rebuild-only. pg_restore DR unaffected. → ✅ **RESOLVED 2026-06-21 (Cluster 2b):** removed the 26-row inputs INSERT from `04_seed_data.sql` (no migration/app/FK dependents; verified). Greenfield now reaches 074 with empty `tenant.inputs`. No prod action (seed runs at mig 006, long past). Static+reasoning verified; definitive gate = scratch-DB `alembic upgrade head` replay (not runnable from the agent env — no docker/TimescaleDB). |
