...
async def create_attestation(body: AttestCreate, db: AsyncSession = Depends(get_tenant_db), user: dict = Depends(get_current_user)):
    ...
    rid = (await db.execute(text("""
        INSERT INTO tenant.attestation_requests
            (tenant_id, requested_by_user_id, claim_type, claim_ref, subject_label,
             verifier_source, verifier_label, token_hash, expires_at)
        VALUES (cast(:t AS uuid), cast(:u AS uuid), :ct, :cr, :sl, :vs, :vl, :th, :exp)
    ...
    """), {"t": str(user["tenant_id"]), "u": str(user["user_id"]), "ct": claim_type, "cr": claim_ref,
           ...
           "exp": _now() + timedelta(days=body.expiry_days)})).scalar()
    ...
    if confirm:
        ...
        await db.execute(text("""
            INSERT INTO tenant.claim_verifications
                (tenant_id, claim_type, claim_ref, source, source_ref, status, confidence_weight, verified_at)
            VALUES (cast(:t AS uuid), :ct, :cr, :src, :sref, 'VERIFIED', :w, now())
            ON CONFLICT (tenant_id, claim_type, claim_ref, source) DO UPDATE SET
                ...
                source_ref=EXCLUDED.source_ref, verified_at=now()
        """), {"t": tenant_id, "ct": r["claim_type"], "cr": r["claim_ref"], "src": r["verifier_source"],
               "sref": (r["verifier_label"] or _SOURCE_LABEL.get(r["verifier_source"])), "w": weight})
    await db.execute(text("""
        UPDATE tenant.attestation_requests SET status=:st, response_note=:n, responded_at=now()
        WHERE request_id=:rid
    """), {"st": "CONFIRMED" if confirm else "DECLINED", "n": note or None, "rid": str(r["request_id"])})
    await db.commit()
