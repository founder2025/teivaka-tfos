...
Revision ID: 028_farmer_label_columns
Revises: 027_seed_productions_catalog
...
Tables modified:
  ...
  - tenant.livestock_register (farmer names their animals)
  - tenant.hive_register      (farmer names their hives)
...
revision = '028_farmer_label_columns'
down_revision = '027_seed_productions_catalog'
branch_labels = None
depends_on = None
...
def upgrade():
    # ------------------------------------------------------------
    # Add farmer_label column to five farmer-visible tenant tables
    ...
    op.execute(
        "ALTER TABLE tenant.livestock_register "
        "ADD COLUMN farmer_label VARCHAR(64)"
    )
    op.execute(
        "ALTER TABLE tenant.hive_register "
        "ADD COLUMN farmer_label VARCHAR(64)"
    )
    op.execute(
    ...
    op.execute(
        "CREATE INDEX idx_livestock_register_farmer_label "
        "ON tenant.livestock_register (tenant_id, farmer_label) "
        "WHERE farmer_label IS NOT NULL"
    )
    op.execute(
        "CREATE INDEX idx_hive_register_farmer_label "
        "ON tenant.hive_register (tenant_id, farmer_label) "
        "WHERE farmer_label IS NOT NULL"
    )
    op.execute(
...
def downgrade():
    ...
    op.execute("DROP INDEX IF EXISTS tenant.idx_hive_register_farmer_label")
    op.execute("DROP INDEX IF EXISTS tenant.idx_livestock_register_farmer_label")
    ...
    op.execute("ALTER TABLE tenant.hive_register DROP COLUMN farmer_label")
    op.execute("ALTER TABLE tenant.livestock_register DROP COLUMN farmer_label")
