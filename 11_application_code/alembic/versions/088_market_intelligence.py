"""088 - Market Intelligence schema (community.*): price/demand/supply records + analytics

Revision ID: 088_market_intelligence
Revises: 087_nursery_batches
Create Date: 2026-06-09

Operator-directed feature (Market Intelligence, folded into the community
Marketplace). Backend architected NOW to support a future national agricultural
intelligence engine (weighted transaction pricing, supply/demand indices,
opportunity scoring, forecasting) WITHOUT a later DB redesign — per the Operator
spec 2026-06-09. V1 exposes Prices / Supply / Demand / Signals / Opportunity from
admin-reference + user-submitted records; the transaction-weighted + forecasting
engine is dormant (fields present, populated later).

Pattern: mirrors community.posts/listings — cross-tenant, NO RLS (access enforced
at the application layer; reads public via get_db, writes authenticated via
get_rls_db which still sets tenant_id for provenance). FKs to tenant.users /
tenant.tenants per the deployment's source-of-truth (no auth schema). production_id
/ variety_id are soft TEXT references to shared.* (no hard FK — shared.* is
read-only at runtime and we don't want inserts to fail on a missing catalog code).

GRANTs: every new community.* table needs explicit GRANT to teivaka_app (B73).

asyncpg: one statement per op.execute via _exec_each (Strike #72).
"""
from alembic import op

revision = "088_market_intelligence"
down_revision = "087_nursery_batches"
branch_labels = None
depends_on = None
...
def upgrade():
    _exec_each([
        ...
        # --------------------------------------------------------------
        # community.price_records — observed prices. Future weighted-market-price
        # engine uses is_actual_sale=TRUE rows only (Σ price*qty / Σ qty). V1 also
        # accepts ADMIN_REFERENCE + USER_SUBMITTED spot prices (is_actual_sale=FALSE).
        # --------------------------------------------------------------
        """
        ...
        # --------------------------------------------------------------
        # community.demand_records — buyer requirements (the Demand Board).
        # Recurring kept distinct from one-off via frequency + is_recurring.
        # --------------------------------------------------------------
        """
        ...
        # --------------------------------------------------------------
        # community.supply_forecasts — projected supply from planting.
        # projected_supply_kg = plants * expected_yield_per_unit_kg * success_probability
        # (or area_ha-based). Stored so the future supply-index engine can aggregate.
        # --------------------------------------------------------------
        """
        ...
        # --------------------------------------------------------------
        # community.market_analytics — computed read-model. V1 computes signals on
        # read; this table lets a future worker materialise national/regional
        # aggregates (weighted price, supply/demand index, balance, opportunity,
        # confidence) keyed by product × variety × grade × location × period.
        # --------------------------------------------------------------
        """
        ...
        # --------------------------------------------------------------
        # Runtime GRANTs — api container connects as teivaka_app (B73).
        # --------------------------------------------------------------
...
def downgrade():
    _exec_each([
        "DROP TABLE IF EXISTS community.market_analytics",
        "DROP TABLE IF EXISTS community.supply_forecasts",
        "DROP TABLE IF EXISTS community.demand_records",
        "DROP TABLE IF EXISTS community.price_records",
    ])
