...
- 2026-06-09: demo_seed.py audited against the real routers BEFORE running (never ran clean yet). Two defects fixed (f45b6c8): (1) `GET /chemicals` now returns `chemical_id` — the canonical `CHEMICAL_APPLIED` (/events) path needs it as the FK the WHD trigger joins on; without it the flagship spray/WHD-block compliance demo silently never seeds; (2) cash `payment_method` MPAISA→MOBILE_MONEY (MPAISA not in the enum → every cash row 422'd; M-PAiSA settlement stays deferred). **Deploy order for the seed run (on the prod host):** (a) `git pull` on parity branch; (b) `docker compose -f 04_environment/docker-compose.yml build --no-cache api && up -d api`; (c) `bash 04_environment/verify-deploy.sh`; (d) `EMAIL=founder@teivaka.com PASSWORD=… BASE_URL=https://teivaka.com python3 scripts/demo_seed.py`; (e) re-run the census query + open Farm › Overview / Compliance. Requires the demo account to have ≥1 ACTIVE cycle (F001 has CASSAVA + EGGPLANT).
