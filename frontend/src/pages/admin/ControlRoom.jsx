    ...
    <AdminLayout>
      <div style={{ maxWidth: 960, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(140px,1fr))", gap: 12, margin: "16px 0" }}>
          {metrics.map(([k, v]) => (
            <div key={k} style={{ background: "var(--paper)", border: `1px solid ${C.line}`, borderRadius: 10, padding: "14px 16px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(240px,1fr))", gap: 12 }}>
          {TOOLS.map((t) => (
            <Link key={t.to} to={t.to} style={{ textDecoration: "none", background: "var(--paper)", border: `1px solid ${C.line}`, borderRadius: 12, padding: 16, display: "flex", gap: 12, alignItems: "center", color: C.soil }}>
