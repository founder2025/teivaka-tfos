...
function RoleBadge({ role }) {
  return (
    <span className="px-2 py-0.5 rounded text-xs bg-gray-700 text-[var(--muted)] uppercase tracking-wide">
      {role}
    </span>
  );
}
  ...
  return (
    <AdminLayout>
      <div className="flex items-center justify-between mb-5">
        <h1 className="text-xl font-bold text-[var(--soil)]">Users <span className="text-gray-500 text-sm font-normal ml-1">({total.toLocaleString()})</span></h1>
        <button
          disabled={selected.size === 0}
          className="text-sm px-3 py-1.5 rounded-lg border border-gray-600 text-[var(--muted)] hover:text-[var(--soil)] disabled:opacity-40 disabled:cursor-not-allowed"
        >
      ...
      <div className="flex flex-wrap gap-2 mb-4">
          ...
          {STATUS_FILTERS.map((f) => (
            <button
              ...
              className={`px-3 py-1.5 text-xs font-medium capitalize transition-colors ${
                filter === f ? "bg-amber-500 text-amber-950" : "text-[var(--muted)] hover:text-[var(--soil)]"
              }`}
            >
              {f}
            </button>
        ...
        <input
          ...
          className="flex-1 min-w-40 bg-white border border-[var(--line)] rounded-lg px-3 py-1.5 text-sm text-[var(--soil)] placeholder-gray-500 focus:outline-none focus:border-amber-500"
        />
      </div>
      ...
      <div className="bg-white border border-[var(--line)] rounded-xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--line)] text-[var(--muted)] text-xs uppercase tracking-wide">
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={u.user_id} className="border-b border-[var(--line)]/50 hover:bg-gray-750 transition-colors">
                  ...
                  <td className="px-3 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-[var(--soil)] font-medium">{u.full_name}</p>
                        <p className="text-gray-500 text-xs">{u.email}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-3 py-3"><RoleBadge role={u.role} /></td>
                  <td className="px-3 py-3 text-[var(--muted)]">{u.country || "—"}</td>
                  <td className="px-3 py-3 text-[var(--muted)] text-xs">
        ...
        {/* Pagination */}
        <div className="flex items-center justify-between px-4 py-3 border-t border-[var(--line)] text-xs text-[var(--muted)]">
          <span>Showing {users.length} of {total.toLocaleString()} users</span>
          <div className="flex gap-2">
            <button disabled={page <= 1} onClick={() => setPage(p => p - 1)}
              className="px-3 py-1 rounded border border-[var(--line)] disabled:opacity-40 hover:text-[var(--soil)]">
              ← Prev
            </button>
