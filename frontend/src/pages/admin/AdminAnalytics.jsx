...
function MetricCard({ label, value, sub, color }) {
  return (
    <div className="bg-white border border-[var(--line)] rounded-xl p-4">
      ...
      <p className={`text-2xl font-bold ${color || "text-[var(--soil)]"}`}>{value ?? "—"}</p>
    ...
    <div className="space-y-1.5">
      {data.slice(0, 10).map((row, i) => (
        <div key={i} className="flex items-center gap-2">
          <span className="text-xs text-[var(--muted)] w-20 truncate shrink-0">{row[labelKey]}</span>
          ...
          <span className="text-xs text-[var(--muted)] w-8 text-right">{row[valueKey]}</span>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <AdminLayout>
      <div className="flex items-center justify-between mb-5">
        <h1 className="text-xl font-bold text-[var(--soil)]">Platform Analytics</h1>
        <div className="flex gap-2">
          {[7, 30, 90].map(d => (
            <button key={d} onClick={() => setDays(d)}
              className={`text-xs px-3 py-1.5 rounded-lg border transition-colors ${
                days === d ? "border-amber-500 bg-amber-500/20 text-amber-400" : "border-[var(--line)] text-[var(--muted)] hover:text-[var(--soil)]"
              }`}>
              {d}d
            </button>
          ))}
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          {/* Signups daily */}
          <div className="bg-white border border-[var(--line)] rounded-xl p-4">
            <h3 className="font-semibold text-[var(--soil)] mb-3">Daily Signups ({days}d)</h3>
          ...
          {/* Signups by country */}
          <div className="bg-white border border-[var(--line)] rounded-xl p-4">
            <h3 className="font-semibold text-[var(--soil)] mb-3">Signups by Country</h3>
            <SimpleBar data={data?.signups_by_country} labelKey="country" valueKey="count" color="bg-emerald-500" />
          </div>

          {/* TIS queries daily */}
          <div className="bg-white border border-[var(--line)] rounded-xl p-4">
            <h3 className="font-semibold text-[var(--soil)] mb-3">AI Queries per Day</h3>
          ...
          {/* Subscription breakdown */}
          <div className="bg-white border border-[var(--line)] rounded-xl p-4">
            <h3 className="font-semibold text-[var(--soil)] mb-3">Subscription Breakdown</h3>
            <div className="space-y-2">
              {[
                ...
                <div key={tier} className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-[var(--muted)] w-28">{tier}</span>
                  <div className="flex-1 bg-gray-700 rounded-full h-2" />
