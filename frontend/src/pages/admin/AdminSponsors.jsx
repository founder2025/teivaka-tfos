...
const inp = { width: "100%", border: "1px solid var(--line)", borderRadius: 8, padding: "8px 10px", fontSize: 13, marginBottom: 8, outline: "none", boxSizing: "border-box" };
  ...
  return (
    <AdminLayout>
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "8px 4px" }}>
        <h1 style={{ fontSize: 20, fontWeight: 800, color: "var(--soil)", marginBottom: 4 }}>Sponsor Corner</h1>
        <p style={{ fontSize: 13, color: "var(--muted)", marginBottom: 18 }}>Sponsored placements shown on the Home feed right rail. Country target empty = everyone.</p>

        <div style={{ background: "var(--paper)", border: "1px solid var(--line)", borderRadius: 12, padding: 16, marginBottom: 22 }}>
          <div style={{ fontWeight: 700, color: "var(--soil)", marginBottom: 12 }}>New placement</div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            ...
            <label style={{ fontSize: 11, color: "var(--muted)" }}>Starts<input type="datetime-local" value={f.starts_at} onChange={(e) => setF({ ...f, starts_at: e.target.value })} style={inp} /></label>
            <label style={{ fontSize: 11, color: "var(--muted)" }}>Ends<input type="datetime-local" value={f.ends_at} onChange={(e) => setF({ ...f, ends_at: e.target.value })} style={inp} /></label>
          </div>
          <button onClick={create} disabled={busy} style={{ background: "var(--green)", color: "#fff", border: "none", borderRadius: 8, padding: "9px 18px", fontWeight: 700, cursor: "pointer" }}>{busy ? "Creating…" : "Create placement"}</button>
        </div>

        <div style={{ fontWeight: 700, color: "var(--soil)", marginBottom: 10 }}>Rate card (FJD) — self-serve ad prices</div>
        <div style={{ background: "var(--paper)", border: "1px solid var(--line)", borderRadius: 10, padding: "10px 14px", marginBottom: 22, display: "flex", flexWrap: "wrap", gap: 16 }}>
          {rates == null ? <span style={{ color: "var(--muted)", fontSize: 13 }}>Loading…</span>
            : rates.map((rt) => (
              <div key={rt.rate_id} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <span style={{ fontSize: 12, color: "var(--muted)", textTransform: "capitalize", minWidth: 56 }}>{rt.billing_period.toLowerCase()}</span>
                <input type="number" defaultValue={rt.price_fjd} onBlur={(e) => { if (Number(e.target.value) !== rt.price_fjd) saveRate(rt.rate_id, e.target.value, rt.active); }} style={{ width: 80, border: "1px solid var(--line)", borderRadius: 6, padding: "5px 8px", fontSize: 13 }} />
                <label style={{ fontSize: 11, color: "var(--muted)", display: "flex", alignItems: "center", gap: 3 }}><input type="checkbox" checked={rt.active} onChange={(e) => saveRate(rt.rate_id, rt.price_fjd, e.target.checked)} /> on</label>
              </div>
            ))}
        </div>

        <div style={{ fontWeight: 700, color: "var(--soil)", marginBottom: 10 }}>Placements & ad requests</div>
        {rows == null ? <div style={{ color: "var(--muted)" }}>Loading…</div>
          : rows.length === 0 ? <div style={{ color: "var(--muted)", fontSize: 13 }}>No placements yet.</div>
            : rows.map((r) => {
              const selfServe = !!r.owner_user_id;
              return (
                <div key={r.placement_id} style={{ background: "var(--paper)", border: `1px solid ${r.status === "PENDING_REVIEW" ? "var(--amber)" : "var(--line)"}`, borderRadius: 10, padding: "12px 14px", marginBottom: 10, display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
                  {r.sponsor_logo && <img src={r.sponsor_logo} alt="" style={{ width: 36, height: 36, borderRadius: 6, objectFit: "cover" }} />}
                  <div style={{ flex: 1, minWidth: 180 }}>
                    <div style={{ fontWeight: 700, color: "var(--soil)", fontSize: 14 }}>{r.title} {selfServe && <span style={{ fontSize: 10, fontWeight: 700, color: "var(--green-dk)", background: "var(--green-tint)", borderRadius: 4, padding: "1px 6px", marginLeft: 6 }}>SELF-SERVE</span>}</div>
                  ...
                  {r.status === "PENDING_REVIEW" && <>
                    <button onClick={() => approve(r.placement_id)} style={{ background: "var(--green)", color: "#fff", border: "none", borderRadius: 6, padding: "5px 12px", fontSize: 12, cursor: "pointer" }}>Approve</button>
                    <button onClick={() => reject(r.placement_id)} style={{ background: "transparent", border: "1px solid var(--amber)", color: "var(--amber)", borderRadius: 6, padding: "5px 10px", fontSize: 12, cursor: "pointer" }}>Reject</button>
                  </>}
                  {r.status === "PENDING_PAYMENT" && <button onClick={() => markPaid(r.placement_id)} style={{ background: "var(--green-dk)", color: "#fff", border: "none", borderRadius: 6, padding: "5px 12px", fontSize: 12, cursor: "pointer" }}>Mark paid → activate</button>}
                  {["ACTIVE", "PAUSED", "ENDED"].includes(r.status) && (
                    <select value={r.status} onChange={(e) => patch(r.placement_id, { status: e.target.value })} style={{ border: "1px solid var(--line)", borderRadius: 6, padding: "5px 8px", fontSize: 12 }}>
                      <option value="ACTIVE">Active</option>
                      <option value="PAUSED">Paused</option>
                      <option value="ENDED">Ended</option>
                    </select>
                  )}
                  <button onClick={() => del(r.placement_id)} style={{ background: "transparent", border: "1px solid var(--red)", color: "var(--red)", borderRadius: 6, padding: "5px 10px", fontSize: 12, cursor: "pointer" }}>Delete</button>
                </div>
              );
            })}
      </div>
