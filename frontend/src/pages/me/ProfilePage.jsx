      ...
      <main style={{ width: "100%", minWidth: 0 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", margin: "16px 0" }}>
          {isYou ? <>
            ...
            <button onClick={() => { if (p.is_following || p.is_connected || p.is_matched) { chat.openWith({ user_id: p.user_id, full_name: p.full_name, profession: p.profession }); chat.setDropdownOpen?.(false); } else alert(`Follow ${p.full_name?.split(" ")[0] || "them"} first to start a conversation.`); }} style={{ background: "var(--paper)", color: (p.is_following || p.is_connected || p.is_matched) ? C.greenDk : C.muted, border: `1px solid ${C.line}`, borderRadius: 8, padding: "9px 16px", cursor: "pointer", display: "inline-flex", gap: 6, alignItems: "center" }}><MessageCircle size={14} />Message</button>
          </>}
        </div>
