...
export default function TISUsage() {
  ...
  const [d, setD] = useState({ calls_used: 0, limit: 0, calls_remaining: 0 });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!alive) return;
        setD(body?.data || { calls_used: 0, limit: 0, calls_remaining: 0 });
        setState("ready");
      } catch {
  ...
  const used = Number(d.calls_used ?? d.calls_today) || 0;
  ...
  return (
    <div className="flex flex-col" style={{ gap: 20 }}>
      <div>
        ...
        <p style={{ fontSize: 13, color: C.soil, margin: "4px 0 0" }}>Your TIS questions this month, against your plan's monthly limit.</p>
      </div>
        ...
        <>
          <div style={{ background: C.paper, border: `1px solid ${C.line}`, borderRadius: 12, padding: 22 }}>
            <div className="flex items-center justify-between" style={{ marginBottom: 12 }}>
              <span style={{ fontSize: 13, color: C.soil, display: "inline-flex", alignItems: "center", gap: 6 }}>
                <Activity size={15} color={C.green} /> This month's questions
              </span>
            ...
            <p style={{ fontSize: 12.5, color: low ? C.amber : C.muted, marginTop: 10 }}>
              {limit > 0 ? `${d.calls_remaining} question${d.calls_remaining === 1 ? "" : "s"} left this month` : "Unlimited on your current plan"}
            </p>
          </div>
          ...
          <div style={{ background: C.greenTint, border: `1px solid ${C.green}`, borderRadius: 12, padding: "14px 16px", display: "flex", gap: 10, alignItems: "flex-start" }}>
            ...
            <p style={{ fontSize: 13, color: C.soil, lineHeight: 1.5, margin: 0 }}>
              Your limit resets at the start of each month. Need more? Upgrade your plan for a higher TIS allowance.
            </p>
          </div>
        </>
      )}
