/**
 * Labor.jsx — /farm/labor (Resources › Labour tab) — audit-approved redesign + 2 optimise passes (2026-06-26).
 *
 ...
 *
 * Redesign (audit L1–L32, L-BUG1) + stress fixes (LS1,LS3,LS8,LS10,LS11b,LS13):
 *  · reads via getJSON / writes via send (token refresh + humanized errors, L4); Fiji day (L5/LS13)
 *  · cached-on-error: never a false "no workers" (L1); banner fires on workers OR labor error (LS1)
 *  · removed undefined onSiteIds (L-BUG1) + fabricated "Next payday" tile (L2)
 *  · pay defaults to THIS WEEK's logged wages, not all-time (L24); "Wages (logged)" honest (L16)
 *  · one primary action per card; dead "Assign task" gone (L18/L20); min-wage soft guard (L30)
 *  · shared <Modal>: Esc-close + role=dialog + focus (LS8) and submit-lock vs double-book (LS11b)
 *  · responsive header + wrapping list rows (LS3); real first-run state (LS10); view-aware Ask AI
 *  · tabs are buttons w/ arrow-key nav (L9/LS8)
 ...
 *  offline write queue (LS2), /labor server limit (LS6/L14), true request idempotency (LS11b).
 */
import { useEffect, useMemo, useRef, useState } from "react";
import { QueryClient, QueryClientProvider, useQuery, useQueryClient } from "@tanstack/react-query";
import { useNavigate } from "react-router-dom";
import { Plus, Search, Pencil, X, MapPin, Sparkles, AlertTriangle, Users } from "lucide-react";
...
const AI_PROMPTS = {
  today: "How can I manage my farm workers and attendance more efficiently?",
  roster: "How should I structure my farm workforce (permanent vs casual vs family)?",
  payroll: "Based on hours logged, who should I pay this week and roughly how much?",
  costing: "How do I work out my labour cost per crop cycle?",
  analytics: "How do I measure and improve worker productivity on my farm?",
};
...
// Shared modal: Esc-to-close, role=dialog/aria-modal, focuses on open (LS8). Removes repeated overlay markup.
...
function FirstRun({ onAdd }) {
  return (
    <div className="card" style={{ padding: "36px 24px", textAlign: "center", marginTop: 14 }}>
      ...
      <div style={{ fontWeight: 700, color: "var(--soil)", fontSize: 16 }}>Add your first worker</div>
      <div style={{ fontSize: 12.5, color: "var(--muted)", margin: "6px auto 16px", maxWidth: 360, lineHeight: 1.5 }}>Track who works the farm, their GPS-verified attendance and wages — all hash-chained for Bank Evidence.</div>
      <button className="btn btn-primary" onClick={onAdd}><Plus size={14} />Add worker</button>
    </div>
  );
}
...
function LaborInner() {
  ...
  const wkAgo = Date.parse(todayISO()) - 6 * 864e5; // last 7 Fiji days incl. today (LS13)
  ...
  const askAi = () => navigate("/tis?q=" + encodeURIComponent(AI_PROMPTS[view] || AI_PROMPTS.today));
  const onTabKey = (e, id) => {
  ...
  const dataDegraded = (workersQ.isError || laborQ.isError) && workers.length > 0; // LS1
    ...
    <TfpShell>
      <main className="main-content">
        <div className="main-inner">
          ...
          <div className="cycle-view-tabs" role="tablist" aria-label="Labour views">
          ...
          {!farmId ? <div className="card" style={{ padding: 20, color: "var(--muted)" }}>Select a farm to see its team.</div>
            ...
            : (
            <>
              {dataDegraded && <DegradedBanner msg={laborQ.isError ? "Couldn't load hours/wages — figures may be incomplete. Showing what loaded." : undefined} />}
              {view === "today" ? (
              team.length === 0 && family.length === 0 ? <FirstRun onAdd={() => setAddOpen(true)} />
              : <>
              ...
              <>
                ...
                <div style={{ marginBottom: 14, position: "relative" }}>
                  <input type="search" value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search workers by name or ID..." aria-label="Search workers" style={{ width: "100%", padding: "9px 12px 9px 38px", border: "1.5px solid var(--line)", borderRadius: 7, fontSize: 13, background: "var(--paper)" }} />
              ...
              <>
                ...
                {attendanceQ.isLoading ? <div className="card" style={{ padding: 16, color: "var(--muted)" }}>Loading…</div>
                  ...
                  : <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>{clockFeed.map((c) => (
                    <div key={c.attendance_id} className="card" style={{ padding: "9px 13px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap", rowGap: 4 }}>
                ...
                {laborQ.isError && records.length === 0 ? <ErrorCard msg="Couldn't load logged days." onRetry={() => laborQ.refetch()} />
                  ...
                  : <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>{records.map((r) => (
                    <div key={r.attendance_id} className="card" style={{ padding: "10px 14px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap", rowGap: 4 }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}><div style={{ fontWeight: 700, color: "var(--soil)" }}>{fjd2(r.total_pay_fjd)}</div><button className="btn btn-secondary btn-sm" title="Fix this day" aria-label="Fix this day" onClick={() => setEditDay(r)}><Pencil size={11} /></button></div>
                    </div>
                  ))}</div>}
              </>
            ) : view === "payroll" ? (
              team.length === 0 ? <Building title="Payroll" body="Wages owed and paid appear here once you add paid workers and mark attendance." />
                : <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  ...
                  {team.map((w) => (
                    <div key={w.worker_id} className="card" style={{ padding: "10px 14px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap", rowGap: 6 }}>
...
function AddWorkerModal({ farmId, onClose, onSaved }) {
  ...
  async function submit() {
    ...
    } catch (e) { emitToast(e?.userMessage || "Could not add worker"); lock.current = false; } finally { setBusy(false); }
  }
  return (
    <Modal title="Add worker" onClose={onClose} foot={<><button className="btn btn-secondary" onClick={onClose}>Cancel</button><button className="btn btn-primary" onClick={submit} disabled={busy}>{busy ? "Adding…" : "Add worker"}</button></>}>
...
function EditRateModal({ worker, onClose, onSaved }) {
  ...
  async function submit() {
    ...
    } catch (e) { emitToast(e?.userMessage || "Could not update worker"); lock.current = false; } finally { setBusy(false); }
  }
  return (
    <Modal title={`Edit worker — ${worker.full_name}`} onClose={onClose} foot={<><button className="btn btn-secondary" onClick={onClose}>Cancel</button><button className="btn btn-primary" onClick={submit} disabled={busy}>{busy ? "Saving…" : "Save"}</button></>}>
...
function DayEditModal({ record, onClose, onSaved }) {
  ...
  async function submit() {
    ...
    } catch (e) { emitToast(e?.userMessage || "Could not save"); lock.current = false; } finally { setBusy(false); }
  }
  return (
    <Modal title={`Fix logged day — ${record.worker_name}`} onClose={onClose} maxWidth={460} foot={<><button className="btn btn-secondary" onClick={onClose}>Cancel</button><button className="btn btn-primary" onClick={submit} disabled={busy}>{busy ? "Saving…" : "Save"}</button></>}>
...
function MarkAttendanceModal({ farmId, workers, preset, onClose, onSaved }) {
  ...
  async function submit() {
    ...
    } catch (e) { emitToast(e?.userMessage || "Could not log attendance"); lock.current = false; } finally { setBusy(false); }
  }
  return (
    <Modal title="Mark attendance" onClose={onClose} foot={<><button className="btn btn-secondary" onClick={onClose}>Cancel</button><button className="btn btn-primary" onClick={submit} disabled={busy || !workerId}>{busy ? "Logging…" : fam ? "Log day (unpaid)" : `Log · ${fjd2(total)}`}</button></>}>
...
function PayWagesModal({ farmId, worker, onClose, onSaved }) {
  ...
  const [busy, setBusy] = useState(false);
  const lock = useRef(false); // submit-lock vs double-book (LS11b)
  async function submit() {
    if (lock.current) return;
  ...
  return (
    <Modal title={`Pay wages — ${worker.full_name}`} onClose={onClose} foot={<><button className="btn btn-secondary" onClick={onClose}>Cancel</button><button className="btn btn-primary" onClick={submit} disabled={busy}>{busy ? "Paying…" : "Pay wages"}</button></>}>
...
function WorkerDetail({ worker, records, att, clockFeed, onClose, onClock, clocking, onPay, onEditRate }) {
  ...
  return (
    <Modal title={worker.full_name} onClose={onClose} maxWidth={620} foot={<>{!fam && <button className="btn btn-secondary" onClick={onPay}>Pay wages</button>}<button className="btn btn-primary" onClick={onClose}>Close</button></>}>
