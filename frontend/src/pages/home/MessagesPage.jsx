 ...
 * Mobile: list → tap → full-screen conversation → back.
 */
...
export default function MessagesPage() {
  ...
  // refresh the list instantly when the SSE stream signals chat activity
  useEffect(() => {
    const on = () => load();
  ...
  const selected = useMemo(() => (conns || []).find((c) => c.user_id === userId) || (userId ? { user_id: userId, full_name: "Conversation", profession: "farmer" } : null), [conns, userId]);

  const Row = (c) => (
    <button key={c.user_id} onClick={() => navigate(`/messages/${c.user_id}`)}
      style={{ width: "100%", display: "flex", gap: 10, alignItems: "center", padding: "11px 12px", border: "none", borderBottom: `1px solid ${C.line}`, background: c.user_id === userId ? "rgba(106,168,79,0.10)" : c.unread > 0 ? "rgba(106,168,79,0.05)" : "#fff", cursor: "pointer", textAlign: "left" }}>
  ...
  const ConvHeader = ({ withBack }) => (
    <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderBottom: `1px solid ${C.line}`, background: C.cream }}>
      ...
      <div style={{ minWidth: 0 }}>
        <div style={{ fontWeight: 700, fontSize: 14.5, color: C.soil }}>{selected?.full_name}</div>
  ...
  const listPane = (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", borderRight: narrow ? "none" : `1px solid ${C.line}` }}>
    ...
    <div className="tfp">
      <main className="main-content">
        <div className="main-inner" style={{ maxWidth: 1100 }}>
          <div className="page-header"><h1>Messages</h1><p className="subtitle">Everyone you're talking to, in one place.</p></div>
          <div className="card" style={{ padding: 0, overflow: "hidden", height: "calc(100vh - 230px)", minHeight: 460 }}>
            {narrow ? (
              userId ? (
                <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
                  <ConvHeader withBack />
                  <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}><Convo conn={selected} onActivity={load} /></div>
                </div>
              ) : listPane
            ) : (
              <div style={{ display: "grid", gridTemplateColumns: "320px 1fr", height: "100%" }}>
                ...
                {selected ? (
                  ...
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", color: C.muted, gap: 8 }}>
